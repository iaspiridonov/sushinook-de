...
+function ($) {
  ...
  var reset_ings = function(){
  	$('.ingBlock').removeClass('ingDisabled');
  	$('.ingBlock').attr('data-active','false');
    $('.ingBlock').find('[type=checkbox]').prop('checked', false);

    $('.post_ing').each(function(){
    	var $ing_el = $(this);
    	if(!$ing_el.hasClass('ingRemove')) $ing_el.remove();
    	else $ing_el.removeClass('ing_active');
    });
 ...
 */
 ...
 //            if(width < 796){
 //                alert();
 //                $('.no_slider').each(function(){
 //                    $(this).data("role", "no");
 //                    $(this).attr("data-role", "no");
 //                });
 //            }
 //        });
(function (global, factory) {
  ...
  var Lazy = {
    loadInSlide: function loadInSlide(index, loadInDuplicate) {
      ...
      $images.each(function (imageIndex, imageEl) {
        ...
        if (window.getComputedStyle($imageEl[0])['display']==='block' || window.getComputedStyle($imageEl[0])['display']==='inline-block') {
...
//# sourceMappingURL=_swiper.mod.js.map
...
(function() {
  ...
  var load = function() {
    ...
    $("[data-role=\"lazyimg\"]").filter(":not(.loaded)").find(".lazy").Lazy({
      ...
      afterLoad: function(element) {
        ...
        $(element).parents("[data-role=\"item\"]").removeClass("loading");
      }
    });
  ...
  $(document)
    .delegate("[data-toggle=\"tab\"]", "shown.bs.tab", function() {
      update();
    })
    .delegate(".modal", "shown.bs.modal", function() {
      update();
    })
    .bind("itemLoadedEvent", function() {
      update();
    });
...
// 	var $body = $("body");
// 	var $elements = $("[data-role=\"header-sup\"], [data-role=\"header-sub\"]");

// 	var set = function() {
// 		$elements.css("padding-right", scroll_width);
// 	};

// 	var unset = function() {
// 		$elements.css("padding-right", 0);
// 	};
...
/*
	модальные окна - открытие второго окна при открытом первом
	--------------------------
	использованные библиотеки:
	- https://getbootstrap.com/docs/3.3/javascript/#modals
	-------------------------------------------------------- */
(function() {
	...
	$(".modal")
		...
		.on("hide.bs.modal", function() {
			if ($(".modal.in")["length"]) {
				$body.addClass("show--clone");
				if (!$(".modal-over")["length"]) $body.append("<div class=\"modal-over\" />");
			}
			$(this).removeClass("modal--clone")
		})
		.on("hidden.bs.modal", function() {
			if ($(".modal.in")["length"]) {
				$body
					.addClass("modal-open show--clone")
					.css("padding-right", scroll_width);
			} else {
				$body
					.removeClass("show--clone")
					.css("padding-right", 0);
				$(".modal-over").remove();
			}
			$(this).removeClass("modal--clone");
...
//# sourceMappingURL=_modals.js.map
...
(function() {
	...
	$.each(["desktop", "laptop", "tablet", "phone"], function(i, flag) {
		var $device = $("[data-device="+ flag +"]");

		var resize = function() {

			if ($device.is(":visible") && DEVICE!=flag) {
				DEVICE = flag;
				$(document).trigger("eventDevice");
			}
...
// --------------------------------------------
//# sourceMappingURL=_device.js.map
...
(function() {
	...
	var $btn 			= $("[data-button=\"nav\"]");
	var $overlay 	= $("[data-role=\"overlay\"]");
  var opened 		= $body.hasClass("show--nav");
  ...
  var close = function(silent_mode) {
    opened = false;
    ...
    if (silent_mode) {
      ...
      $overlay.fadeOut();
      $nav.one("bsTransitionEnd", function() {
	      $body.removeClass("hide--nav");
      });
    }
  };
  ...
  var resize = function() {
    if (DEVICE!="phone" && opened) close(true);
  };
...
/*
	шапка сайта / фиксация при скролле
	------------------------------------
	использованные библиотеки:
 	------------------------------------ */
(function() {
	...
	var $header_sup = $("[data-role=\"header-sup\"]");
	var $header_sub = $("[data-role=\"header-sub\"]");

	var scroll = function() {
		var y 			= $win.scrollTop();
		var offset 	= $header_sup.height();
...
(function() {
	...
	var $section = $("[data-role=\"promo\"]");

	if (!$section["length"]) return false;

	var $slider = $section.find("[data-role=\"slider\"]");
	...
	var slider = new Swiper($slider, {
		...
		breakpoints: {
			1024: { spaceBetween:20, slidesPerView: "auto", centeredSlides: true, },
			768: { spaceBetween:20, slidesPerView: "auto", centeredSlides: true, }
		},		
	});
...
(function() {
	...
	$('.catalog-item-types [type=radio]').change(function(){
		var el = $(this);
		var typeID = el.attr('id');
		var typeText = $('[for='+typeID+']').html();
		el.parents('.modal_right').find('.pizzaType').text(typeText.toLowerCase());
		changePriceT(el);
	});

	var changePriceT = function($el){
		var $item = $el.parents('[data-role="item"]');
		var price = $item.find('[data-price]').filter(':checked').data('price');
		var type = $item.find('.catalog-item-types [type=radio]:checked + label').text();
		if(type == 'Тонкое') price = $item.find('[data-price]').filter(':checked').data('price-t');
		$('.ingBlock[data-active="true"]').each(function(){
			var block = $(this);
			var sizePrice = block.data('price-'+size);
			if(sizePrice){
				price += sizePrice;
			}
		});
		...
		if (price){
			$item.find('[data-output="amount"]').text(number_format(price, 2, ".", " "));
			$item.find('[name="price"]').val(price);
			$item.attr('data-price',price);
			$item.find('.ingridientsAdd').attr('data-price',price);
		}
	}

	$("[data-role=\"catalog\"]").each(function() {
		var $section 	= $(this);
		var $items 		= $section.find("[data-role=\"item\"]");
		...
		$items.each(function() {
			...
			var output = {
				amount: $item.find("[data-output=\"amount\"]"),
				count: 	$item.find("[data-output=\"count\"]")
			};
			var inps = {
				count: 	$item.find("[name=\"count\"]"),
				price: 	$item.find("[name=\"price\"]")
			};
			var btns = {
				minus: 	$item.find("[data-button=\"minus\"]"),
				plus: 	$item.find("[data-button=\"plus\"]")
			};

			var price = inps["price"].val();
			var count = inps["count"].val();

			var calc = function() {
				var price = $item.find('[name="price"]').val();
				var amount = price*count;

				btns["minus"].attr("disabled", count==1);

				output["count"].text(count);
				output["amount"].text(number_format(price, 2, ".", " "));

				inps["count"].val(count);
			};

			var changeSize = function() {
				var size = $item.find("[data-size]").filter(":checked").data("size");
				if(size){
					$item.find('.pizzaSize').text(size);
					if(size == 25){
						$item.find('.catalog-item-types [type=radio]:first').prop('checked', true);
						$item.find('.catalog-item-types [type=radio]:nth-of-type(2)').attr('disabled','disabled');
						$item.find('.catalog-item-meta').find('.pizzaType').text($item.find('.catalog-item-types [type=radio]:checked').data('text'));
					}else{
						// $('.catalog-item-meta').find('.pizzaType').text('тонкое');
						$item.find('.catalog-item-types [type=radio]:nth-of-type(2)').removeAttr('disabled');
					}
				}
				
				// if (size) $item.attr("data-size", size);
			};

			var changePrice = function(){
				var size = $item.find('[data-price]').filter(':checked').data('size');
				if(size){
					$('.ingBlock').each(function(){
						var block = $(this);
						var sizePrice = block.data('price-'+size);
						if(sizePrice){
							block.attr('data-price',sizePrice);
							block.find('[name="ingPrice"]').val(sizePrice);
							block.find('.addon-value').html(sizePrice+' тг.');
						}
					});
				}
			...
			btns["minus"].on("click", function() {
				count--;
				calc();
			});

			btns["plus"].on("click", function() {
				count++;
				calc();
			});

			$item.find("[data-size]").on("change", function() {
				changeSize();
				changePrice();
				var settingID = $(this).data('setting');
				var parent = $(this).parents('.modal_right');
				$(parent).find('.catalog-item-types').hide();
				$(parent).find('#testo-' + settingID).show();

				$(parent).find('.catalog-item-types [type=radio]').attr('disabled', true).attr('checked', false);

				$(parent).find('#testo-' + settingID).find('[type=radio]').attr('disabled', false);

				$(parent).find('#testo-' + settingID).find('[type=radio]:first').prop('checked', true);

				$(this).parents('.modal_right').find('.pizzaType').text($(parent).find('#testo-' + settingID).find('[type=radio]:first').data('text'));


			});

			var calc2 = function(){
			    $('.item--pizza').each(function(){
			        var block = $(this);
			        var price = block.find('input[name="price"]').val();
			...
			$item.find('.ingBlock [type="checkbox"]').on("change", function(){
		        ...
		        var i = 0;
		        $('.ingridientsAdd .additionalIng[data-id="'+blockID+'"] .ingBlock[data-active="true"]').each(function(){
		            i++;
		        });

		        var prodPrice = block.parents('.item--pizza').find('[name="price"]').val();
		        prodPrice = parseInt(prodPrice);
		        var curIngPrice = block.find('[name="ingPrice"]').val();
		        curIngPrice = parseInt(curIngPrice);


		        if(!isChecked){
		        	var addBlock = $('[data-id="post_ing'+id+'"]');
		        	addBlock.remove();
		        	prodPrice -= curIngPrice;
		        }else{
		        	$('.post_ings[data-id="'+blockID+'"]').append( $('<div/>',{attr:{class:'post_ing','data-id':'post_ing'+id}}).append( $('<strong/>',{html:'+ '+name}) ) );
		        	prodPrice += curIngPrice;
		        }
		        ...
		        var prodID = block.parents('.ingridientsAdd').data('id');
		        
		        $('.IngAddBtn[data-id="'+blockID+'"] sup').html(i);
		        $('.item--pizza[data-id="'+prodID+'"]').find('input[name="price"]').val(prodPrice);
...
(function() {
	...
	$("[data-role=\"catalog\"]").each(function() {
		var $section 	= $(this);
		var $slider 	= $section.find("[data-role=\"slider\"]");
		var $sliderIn = $section.find("[data-role=\"slider-in\"]");
		var $slides 	= $section.find("[data-role=\"slide\"]");
		var slider;

		var init = function() {
			$slider.addClass("swiper-container");
			$sliderIn.addClass("swiper-wrapper");
			...
			slider
		      ...
		      $(slideEl)
		      	.addClass("loaded")
		      	.find("[data-role=\"item\"]").removeClass("loading");
		    });
		};
		...
		var destroy = function() {
			...
			$sliderIn.removeClass("swiper-wrapper");
			$slides.removeClass("swiper-slide");
		};

		var check = function() {
			if (DEVICE=="phone" && !slider) init();
			if (DEVICE!="phone" && slider) destroy();
		};
...
(function() {
	...
	var $section = $("[data-role=\"combo\"]");

	if (!$section["length"]) return false;

	var $resultDiv 	= $section.find("[data-role=\"result\"]");
	var $result 		= $section.find("[data-output=\"result\"]");

	$section.find("[data-role=\"item\"]").each(function() {
		var $item = $(this);
		var price = parseFloat($item.find("[name=\"price\"]").val(), 10);
		$item.find("[data-output=\"price\"]").text(number_format(price, 2, ".", " "));
	});

	var calculate = function() {
		var result = 0;

		$section.find("input[type=\"checkbox\"]").filter(":checked").each(function() {
	...
	$("[data-role=\"combo\"]").find("[data-role=\"tab\"]").each(function() {
		var $tab 		= $(this);
		var tab 		= $tab.attr("id");
		var $items 	= $tab.find("[data-role=\"item\"]");
		var limit 	= $items["length"];
		var $checkboxes = $tab.find("input[type=\"checkbox\"]");
		var $link 	= $("[aria-controls=\""+ tab +"\"]");
		var $scroll = $tab.find("[data-role=\"scroll\"]");
		var scrolls;

		var output = {
			"limit": 		$tab.find("[data-output=\"limit\"]"),
			"count": 		$tab.find("[data-output=\"count\"]"),
			"counter": 	$link.find("[data-output=\"counter\"]")
		};

		// output["limit"].text(limit);

		var scroll = function() {
			if (DEVICE=="phone" && scrolls) {
				$scroll.nanoScroller({
					destroy: true
				});
				scrolls = false;
		...
		var calc = function() {
			...
			output["counter"]
				...
				.addClass(count>0 ? "green" : "")
				.addClass(count==0 ? "red" : "")
				.text(count);

			var comboTabsCount = 0;
			$('.tab-pane').each(function(){
				var block = $(this);
				var max = block.find('[data-output="limit"]').text();
				comboTabsCount += parseInt(max);
			});
			var comboTabsChecked = 0;
			$('.comboTabsItem').each(function(){
				var el = $(this);
	            var itemCount = el.find('[data-output="counter"]').html();
	            comboTabsChecked += parseInt(itemCount);
	        });
	        if(comboTabsCount == comboTabsChecked){
	        	$('.combo-result .amount .btn--primary').removeClass('btnDisabled');
            }else{
            	$('.combo-result .amount .btn--primary').addClass('btnDisabled');
            }
		};
...
(function() {
	...
	var $section = $("[data-role=\"halves\"]");

	if (!$section["length"]) return false;

	var $result = $section.find("[data-role=\"result\"]");

	var lists = {
		"left": 	$section.find("[data-list=\"left\"]"),
		"right": 	$section.find("[data-list=\"right\"]")
	};

	var checkboxes = {
		"left": 	lists["left"].find("[type=\"radio\"]"),
		"right": 	lists["right"].find("[type=\"radio\"]")
	};

	var output = {
		"left": {
			"placeholder": 	$section.find("[data-output=\"placeholder-left\"]"),
			"description": 	$section.find("[data-output=\"description-left\"]"),
			"image": 				$section.find("[data-output=\"image-left\"]")			
		},
		"right": {
			"placeholder": 	$section.find("[data-output=\"placeholder-right\"]"),
			"description": 	$section.find("[data-output=\"description-right\"]"),
			"image": 				$section.find("[data-output=\"image-right\"]")			
		},
		"result": $section.find("[data-output=\"result\"]")
	};

	//пробегаем все пиццы и выставляем цены с учетом разделения на тысячные
	$section.find("[data-role=\"item\"]").each(function() {
		var $item = $(this);
		var price = parseFloat($item.find("[name=\"price\"]").val(), 10);
		$item.find("[data-output=\"price\"]").text(number_format(price, 2, ".", " "));
	...
	var scroll = function() {
		if (DEVICE=="phone" && scrolls) {
			$section.find("[data-role=\"scroll\"]").nanoScroller({
				destroy: true
			});
			scrolls = false;
		}
	...
	//
	var calc = function() {
		//всем радио-кнопка (пиццы) убираем атрибут disabled
		checkboxes["left"].attr("disabled", false);
		checkboxes["right"].attr("disabled", false);

		//выбрана Левая/Правая
		var left, right;

		//Итоговый результат в деньгах
		result = 0;

		//находим выбранный пункт слева
		checkboxes["left"].filter(":checked").each(function() {
			var $item = $(this).parents("[data-role=\"item\"]");
			var id 		= $(this).data("id");
			var price = parseFloat($item.find("[name=\"price\"]").val(), 10);
			var title = $item.find("[data-role=\"title\"]").html();
			var image = $item.data("image");

			//идентичной пицце справа устанавливаем атрибут disabled
			checkboxes["right"].filter("[data-id=\""+ id +"\"]").attr("disabled", true);

			output["left"]["placeholder"].hide();
			output["left"]["description"].show().html("<b>" + title + "</b><br />" + number_format(price, 2, ".", " ") + " тг.");
			output["left"]["image"].show().html("<div><img src=\"" + image + "\" /></div>");
		...
		//находим выбранный пункт справа
		checkboxes["right"].filter(":checked").each(function() {
			var $item = $(this).parents("[data-role=\"item\"]");
			var id 		= $(this).data("id");
			var price = parseFloat($item.find("[name=\"price\"]").val(), 10);
			var title = $item.find("[data-role=\"title\"]").html();
			...
			//идентичной пицце слева устанавливаем атрибут disabled
			checkboxes["left"].filter("[data-id=\""+ id +"\"]").attr("disabled", true);

			output["right"]["placeholder"].hide();
			output["right"]["description"].show().html("<b>" + title + "</b><br />" + number_format(price, 2, ".", " ") + " тг.");
			output["right"]["image"].show().html("<div><img src=\"" + image + "\" /></div>");
		...
		if (result>0 && left && right) {
			$result.show();
			// output["result"].text(number_format(result, 2, ".", " "));
		}
	};

	calc();

	checkboxes["left"].on("change", function() {
		calc();
	});

	checkboxes["right"].on("change", function() {
		calc();
	});
...
/*
	...
	------------------------------------
	использованные библиотеки:
	- https://tech.yandex.com/maps/doc/jsapi/2.1/quick-start/tasks/quick-start-docpage/?from=jsapi
...
(function() {
	...
	var pin 		= {
		"src": 		$map.data("pin"),
		"sizes": 	$map.data("sizes"),
		"offset": $map.data("offset")
	};

	console.log(latlng, zoom, pin);
	...
	var render = function() {
		ymaps.ready(function() {
			...
			var marker = new ymaps.Placemark(latlng, {}, {
				...
				iconImageSize: 		pin["sizes"],
				iconImageOffset: 	pin["offset"]
			});
...
(function() {
	...
	$("[data-role=\"relative\"]").find("[data-role=\"slider\"]").each(function() {
		var $slider = $(this);
		var CID 		= Math.floor(Math.random() * 9999999);
		// var count 	= $slider.find

		$slider
			.find("[data-button=\"prev\"]").addClass("relative-prev-" + CID).end()
			.find("[data-button=\"next\"]").addClass("relative-next-" + CID);
		...
		var slider = new Swiper($slider, {
			...
			spaceBetween: 20,
			navigation: {
				nextEl: ".relative-next-" + CID,
				prevEl: ".relative-prev-" + CID,
				disabledClass: "disabled"
			},
			on: {
				init: function() {
